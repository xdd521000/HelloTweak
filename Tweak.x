...
- (void)viewDidAppear:(BOOL)animated {
    ...
    UILabel *st = [self.view viewWithTag:1003];
    if ([st isKindOfClass:[UILabel class]]) {
        st.text = antiRevokeEnabled() ? @"✅ 微信防撤回：已开启" : @"⚠️ 微信防撤回：未开启（请打开上方开关）";
        st.textColor = antiRevokeEnabled() ? [UIColor colorWithRed:0.4 green:0.9 blue:0.5 alpha:1.0] : [UIColor colorWithRed:0.95 green:0.6 blue:0.3 alpha:1.0];
    }
}
...
- (void)viewDidLoad {
    ...
    // 当前防撤回状态提示（让用户明确知道开关有没有开）
    UILabel *statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 66, self.view.bounds.size.width - 40, 30)];
    statusLabel.text = antiRevokeEnabled() ? @"✅ 微信防撤回：已开启" : @"⚠️ 微信防撤回：未开启（请打开上方开关）";
    statusLabel.font = [UIFont systemFontOfSize:13];
    statusLabel.textColor = antiRevokeEnabled() ? [UIColor colorWithRed:0.4 green:0.9 blue:0.5 alpha:1.0] : [UIColor colorWithRed:0.95 green:0.6 blue:0.3 alpha:1.0];
    statusLabel.textAlignment = NSTextAlignmentCenter;
    statusLabel.tag = 1003;
    ...
    ver.text = @"小叮当 v0.0.14 ｜ 适配 iOS 16.2 / Dopamine";
...
// ============================================================
//  5.0 自适应防撤回（兼容微信新版接口）
//  原理：微信不同版本接收"撤回"回调的方法名可能不同（如 onRevokeMsg:
//  / onRevokeMsgCgiReturn: 等）。这里自动扫描 CMessageMgr 里所有
//  onRevoke* 开头的方法，全部拦截，不再依赖单一方法名，从根本适配。
// ============================================================

static NSMutableDictionary *xddRevokeOrigImps = nil;

static IMP xddRevokeOrigImp(SEL sel) {
    NSValue *v = [xddRevokeOrigImps objectForKey:NSStringFromSelector(sel)];
    return v ? (IMP)[v pointerValue] : NULL;
}

static void xddRevokeHandler0(id self, SEL _cmd) {
    if (antiRevokeEnabled()) {
        if (revokeToastEnabled()) showRecallToast(revokeNoticeText());
        return; // 拦截撤回
    }
    IMP orig = xddRevokeOrigImp(_cmd);
    if (orig) ((void(*)(id, SEL))orig)(self, _cmd);
}

static void xddRevokeHandler1(id self, SEL _cmd, id a1) {
    if (antiRevokeEnabled()) {
        if (revokeToastEnabled()) showRecallToast(revokeNoticeText());
        return; // 拦截撤回
    }
    IMP orig = xddRevokeOrigImp(_cmd);
    if (orig) ((void(*)(id, SEL, id))orig)(self, _cmd, a1);
}

static void xddRevokeHandler2(id self, SEL _cmd, id a1, id a2) {
    if (antiRevokeEnabled()) {
        if (revokeToastEnabled()) showRecallToast(revokeNoticeText());
        return; // 拦截撤回
    }
...
static void xddInstallAdaptiveRevokeHooks(void) {
    Class cls = NSClassFromString(@"CMessageMgr");
    if (!cls) return;
    if (!xddRevokeOrigImps) xddRevokeOrigImps = [NSMutableDictionary dictionary];
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        // 只处理"接收撤回"回调（onRevoke 开头），不误伤发送撤回
        if (![lower hasPrefix:@"onrevoke"]) continue;
        // onRevokeMsg 已由下方 %hook 精确处理，跳过避免重复
        if ([name isEqualToString:@"onRevokeMsg"]) continue;
        NSString *key = NSStringFromSelector(sel);
        if ([xddRevokeOrigImps objectForKey:key]) continue;
        [xddRevokeOrigImps setObject:[NSValue valueWithPointer:method_getImplementation(m)] forKey:key];
        unsigned int nargs = method_getNumberOfArguments(m) - 2; // 去掉 self、_cmd
        IMP newImp = NULL;
        if (nargs == 0) newImp = (IMP)xddRevokeHandler0;
        else if (nargs == 1) newImp = (IMP)xddRevokeHandler1;
        else if (nargs == 2) newImp = (IMP)xddRevokeHandler2;
        else continue;
